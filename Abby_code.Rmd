...
leaflet() %>%
  addTiles() %>%
  addPolylines(data = mac_elevation, weight =1) %>%
  addPolylines(data=macleish_layers[["trails"]], weight = 1, color=~trail_pal)

trail_pal <- colorFactor("Set1", macleish_layers[["trails"]]@data$name)
